    ...
    for (int i = top_left.y; i < top_left.y + height; ++i)
    {
        for (int j = top_left.x; j < top_left.x + width; ++j)
        {
            ...
            for (int s = 0; s < samples_per_pixel; s++)
            {
                ...
                raytracer::Ray r = world.camera->get_ray(raytracer::Point2(u, v));

                pixel_color += tracer->trace_ray(r, world.objects, max_depth);
            }
...
void setup()
{
        ...
        for (int b = -11; b < 11; b++)
        {
            ...
            if ((center - raytracer::Point3(4, 0.2, 0)).Length() > 0.9)
            {
                ...
                if (choose_mat < 0.8)
                {
                    ...
                    world.add_object(std::make_shared<raytracer::Sphere>(center, 0.2, sphere_material));
                }
                else if (choose_mat < 0.95)
                {
                    ...
                    sphere_material = std::make_shared<raytracer::Reflective>(1, cr, 0.0);
                    world.add_object(std::make_shared<raytracer::Sphere>(center, 0.2, sphere_material));
                }
                else
                {
    ...
    world.add_object(std::make_shared<raytracer::Sphere>(raytracer::Point3(0, 1, 0), 1.0, material1));

    auto material2 = std::make_shared<raytracer::Matte>(1.0, raytracer::Color3(0.4, 0.2, 0.1));
    world.add_object(std::make_shared<raytracer::Sphere>(raytracer::Point3(-4, 1, 0), 1.0, material2));

    auto material3 = std::make_shared<raytracer::Reflective>(1, raytracer::Color3(0.7, 0.6, 0.5), 0.0);
    world.add_object(std::make_shared<raytracer::Sphere>(raytracer::Point3(4, 1, 0), 1.0, material3));

    // Camera
    std::shared_ptr<raytracer::Pinhole> camera = std::make_shared<raytracer::Pinhole>(raytracer::Vector3(13, 2, 3), raytracer::Vector3(0, 0, 0));
    camera->set_fov(20);
    camera->compute_pixel_size(image_width, image_height);
    camera->compute_uvw();
    world.set_camera(camera);

    world.objects = raytracer::HittableList(std::make_shared<raytracer::BVH_Node>(world.objects));

    // Anti Aliasing Sampler
    sampler = new raytracer::MultiJittered(100);
